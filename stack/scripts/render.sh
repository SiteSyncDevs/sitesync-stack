...
# ------------------------------------------------------- chirpstack.toml -----
# Turn RF_REGION into the enabled_regions list. Every frequency plan of that
# RF region is enabled: an operator picks "US915", not sixteen file names.
RF_REGION="${RF_REGION:-}"
if [[ -z "$RF_REGION" ]]; then
...
_quoted=""
for _id in "${_ids[@]}"; do _quoted+="${_quoted:+, }\"$_id\""; done
...
if (( ${#_ids[@]} == 1 )); then
  echo "  $RF_REGION: 1 frequency plan enabled"
else
  echo "  $RF_REGION: all ${#_ids[@]} frequency plans enabled"
fi

# ------------------------------------------------- gateway bridge instances ---
# One gateway bridge per entry in GATEWAY_BRIDGES ("subband:port"). Compose
# cannot loop, so the services are generated into a second compose file that
# .env points COMPOSE_FILE at. That makes the number of bridges unlimited --
# a site can serve an 8-channel sub-band and a 16-channel one side by side.
mkdir -p compose
GW_FILE=compose/gateways.yml
{
  echo "# Generated by ./sitesync apply from GATEWAY_BRIDGES in .env."
  echo "# Do not edit. Change GATEWAY_BRIDGES and run ./sitesync apply."
  echo "services:"
} > "$GW_FILE"

_n=0
declare -A _seen_port=()
for _entry in ${GATEWAY_BRIDGES:-}; do
  [[ -n "$_entry" ]] || continue
  _sub="${_entry%%:*}"
  _port="${_entry##*:}"
  if [[ "$_sub" == "$_entry" || -z "$_port" ]]; then
    echo "GATEWAY_BRIDGES entry '$_entry' is malformed. Use subband:port, e.g. us915_0:1700" >&2
    exit 1
  fi
  if ! region_id_exists "$_sub"; then
    echo "GATEWAY_BRIDGES names sub-band '$_sub', which is not a known frequency plan." >&2
    echo "Sub-bands available for $RF_REGION:" >&2
    printf '  %s\n' "${_ids[@]}" >&2
    exit 1
  fi
  if [[ "$(rf_of_region_id "$_sub")" != "$RF_REGION" ]]; then
    echo "GATEWAY_BRIDGES names sub-band '$_sub', which belongs to $(rf_of_region_id "$_sub"), not $RF_REGION." >&2
    echo "A gateway bridge can only serve a sub-band of the RF region this site runs." >&2
    exit 1
  fi
  [[ "$_port" =~ ^[0-9]+$ ]] || { echo "GATEWAY_BRIDGES: '$_port' is not a port number." >&2; exit 1; }
  if [[ -n "${_seen_port[$_port]:-}" ]]; then
    echo "GATEWAY_BRIDGES uses port $_port twice (${_seen_port[$_port]} and $_sub). Each bridge needs its own port." >&2
    exit 1
  fi
  _seen_port[$_port]="$_sub"
  _n=$((_n+1))
...
if (( _n == 0 )); then
  {
    echo "# Generated by ./sitesync apply from GATEWAY_BRIDGES in .env."
    echo "# GATEWAY_BRIDGES is empty, so no gateway bridge is running and no"
    echo "# gateway can reach this site. Set it in .env and re-run apply."
    echo "services: {}"
  } > "$GW_FILE"
  echo "  no gateway bridges configured -- no gateway can connect to this site"
fi
